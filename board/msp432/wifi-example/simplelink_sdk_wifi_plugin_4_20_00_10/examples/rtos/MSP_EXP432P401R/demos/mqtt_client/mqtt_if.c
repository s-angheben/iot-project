                        ...
                        uint32_t dataLen)
{
    ...
    status =
        mq_send(mMQTTContext.msgQueue, (char*)&queueElement,
...
int MQTTHelperTopicMatching(char *s,
                            char *p)
{
...
void * MQTTAppThread(void *threadParams)
{
    ...
    while(1)
    {
        mq_receive(mMQTTContext.msgQueue, (char*)&queueElement,
        ...
        if(queueElement.event == MQTT_EVENT_RECV)
        {
            ...
            if(mMQTTContext.head != NULL)
            {
                ...
                while(ret == 0)
                {
                    ret = MQTTHelperTopicMatching(tmp->topicParams.topic,
                                                  queueElement.topic);
                    if(ret == 1)
                    {
                    ...
                    {
                        LOG_INFO(
                            "Cannot invoke CB for topic not in topic list\r\n");
        ...
        else if(queueElement.event == MQTT_EVENT_CONNACK)
        {
            ...
            if(mMQTTContext.head != NULL)
            {
                ...
                while(curr != NULL)
                {
                    ...
                    ret = MQTTClient_subscribe(mMQTTContext.mqttClient,
                                               &curr->topicParams, 1);
...
int MQTT_IF_Init(MQTT_IF_InitParams_t initParams)
{
    ...
    ret =
        pthread_mutex_init(mMQTTContext.moduleMutex,
...
int MQTT_IF_Deinit(MQTTClient_Handle mqttClientHandle)
{
    ...
    ret =
        mq_send(mMQTTContext.msgQueue, (const char*)&queueElement,
                sizeof(struct msgQueue), 0);
    if(ret < 0)
    {
...
MQTTClient_Handle MQTT_IF_Connect(MQTT_IF_ClientParams_t mqttClientParams,
                                  MQTTClient_ConnParams mqttConnParams,
                                  MQTT_IF_EventCallback_f mqttCB)
{
    int ret;
    pthread_attr_t threadAttr;
    ...
    mMQTTContext.mqttClient = MQTTClient_create(MQTTClientCallback,
                                                &clientParams);
    ...
    ret |= pthread_attr_setdetachstate(&threadAttr, PTHREAD_CREATE_DETACHED);
    ret |= pthread_create(&mqttContextThread, &threadAttr, MQTTContextThread,
                          NULL);
    if(ret != 0)
    {
        pthread_mutex_unlock(mMQTTContext.moduleMutex);
    ...
    {
        MQTTClient_set(mMQTTContext.mqttClient, MQTTClient_WILL_PARAM,
                       mqttClientParams.willParams, sizeof(MQTTClient_Will));
    }

    if(mqttClientParams.username != NULL)
    {
        MQTTClient_set(mMQTTContext.mqttClient, MQTTClient_USER_NAME,
                       mqttClientParams.username,
                       strlen(mqttClientParams.username));
    }

    if(mqttClientParams.password != NULL)
    {
        MQTTClient_set(mMQTTContext.mqttClient, MQTTClient_PASSWORD,
                       mqttClientParams.password,
                       strlen(mqttClientParams.password));
    }

    if(mqttClientParams.cleanConnect == false)
    {
        MQTTClient_set(mMQTTContext.mqttClient, MQTTClient_CLEAN_CONNECT,
                       &mqttClientParams.cleanConnect,
                       sizeof(mqttClientParams.cleanConnect));
    }

    if(mqttClientParams.keepaliveTime > 0)
    {
        MQTTClient_set(mMQTTContext.mqttClient, MQTTClient_KEEPALIVE_TIME,
                       &mqttClientParams.keepaliveTime,
                       sizeof(mqttClientParams.keepaliveTime));
    }
...
int MQTT_IF_Subscribe(MQTTClient_Handle mqttClientHandle,
                      char* topic,
                      unsigned int qos,
                      MQTT_IF_TopicCallback_f topicCB)
{
    ...
    topicEntry->topicParams.topic =
        (char*)malloc((strlen(topic) + 1) * sizeof(char));
...
int MQTT_IF_Unsubscribe(MQTTClient_Handle mqttClientHandle,
                        char* topic)
{
    int ret = 0;
...
int MQTT_IF_Publish(MQTTClient_Handle mqttClientParams,
                    char* topic,
                    char* payload,
