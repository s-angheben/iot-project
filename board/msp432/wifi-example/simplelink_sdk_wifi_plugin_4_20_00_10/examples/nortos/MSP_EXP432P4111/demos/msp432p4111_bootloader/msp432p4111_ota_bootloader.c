...
void mainThread(void *pvParameters)
{
    ...
    uint32_t bytesLeft;
    uint32_t maxFlashAddress = MAP_SysCtl_A_getFlashSize() / 2;
    ...
    GPIO_write(MSP_EXP432P4111_GPIO_LED_BLUE, Board_GPIO_LED_ON);
    ...
    /* Unprotecting all of the sectors that we want to */
    MAP_FlashCtl_A_unprotectMemory(APP_SW_START_ADDRESS,
                                   APP_SW_START_ADDRESS + pFsFileInfo.Len);
    ...
    while(bytesLeft != 0)
    {
        ...
        if(!ROM_FlashCtl_A_programMemory(
    ...
    GPIO_write(MSP_EXP432P4111_GPIO_LED_BLUE, Board_GPIO_LED_OFF);
}
...
static void errorLoop()
{
    ...
    while(1)
    {
        ...
        GPIO_toggle(MSP_EXP432P4111_GPIO_LED_RED);
        GPIO_toggle(MSP_EXP432P4111_GPIO_LED_GREEN);
        GPIO_toggle(MSP_EXP432P4111_GPIO_LED_BLUE);
    }
}
